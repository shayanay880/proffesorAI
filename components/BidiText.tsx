...
import { ROBUST_LTR_REGEX, getTextDir, getLangFromDir } from '../utils/bidi';
...
export const BidiText: React.FC<BidiTextProps> = ({ text, className = '' }) => {
  ...
  const textDir = getTextDir(text);

  return (
    <span className={className} dir={textDir} lang={getLangFromDir(textDir)}>
      {parts.map((part, i) => {
