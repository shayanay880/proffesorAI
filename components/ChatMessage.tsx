...
import { getTextDir, ROBUST_LTR_REGEX } from '../utils/bidi';
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, learnerLevel }) => {
  ...
  const alignmentClass = isUser ? 'max-w-2xl ml-auto' : `max-w-4xl ${textDir === 'rtl' ? 'ml-auto' : ''}`;

  return (
    <div className={`flex w-full mb-8 ${isUser ? 'justify-start' : 'justify-start flex-col'}`} dir={textDir}>
      <div className={`group relative w-full ${alignmentClass}`}>
        ...
        <div className={`p-6 rounded-3xl shadow-sm border transition-all duration-300 relative ${
            ...
            {!isUser && (
                <button
                    onClick={handleCopy}
                    className="absolute top-4 left-4 p-2 bg-slate-50 text-slate-400 rounded-xl opacity-0 group-hover:opacity-100 hover:text-cyan-600 transition-all shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-cyan-500"
