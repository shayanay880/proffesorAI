...
const parseAndRenderText = (text: string) => {
  ...
  const content = blocks.map((block, index) => {
    ...
    if (trimmedBlock.startsWith('**Confidence:**') || trimmedBlock.startsWith('Confidence:')) {
        ...
        return (
            <div key={index} dir={blockDir} lang={blockLang} className={`my-3 p-2 rounded-lg border ${borderColor} ${bgColor} flex items-center text-sm text-gray-700`}>
    ...
    if (trimmedBlock.startsWith('⚠️') || trimmedBlock.startsWith('⛔') || trimmedBlock.startsWith('!!') || trimmedBlock.startsWith('🚨') || trimmedBlock.startsWith('🚫')) {
        ...
        return (
            <div key={index} dir={blockDir} lang={blockLang} className={containerClass}>
    ...
    if (trimmedBlock.startsWith('[EXTRA]')) {
         ...
         return (
             <div key={index} dir={blockDir} lang={blockLang} className="my-2 text-sm leading-relaxed bidi-text">
    ...
    if (lines.length > 1 && lines[0].includes('|') && lines[1].includes('-') && lines[1].includes('|')) {
        ...
        if (headerCells.length > 0 && bodyRows.every(row => row.length === headerCells.length)) {
            return (
                <div key={index} dir={blockDir} lang={blockLang} className="my-4 overflow-x-auto rounded-lg border border-gray-300">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                {headerCells.map((header, hIndex) => (
                                    <th key={hIndex} scope="col" dir={blockDir} lang={blockLang} className="px-4 py-3 text-left text-sm font-semibold text-gray-800 bidi-text">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {bodyRows.map((row, rIndex) => (
                                <tr key={rIndex} className="hover:bg-gray-50">
                                    {row.map((cell, cIndex) => (
                                        <td key={cIndex} dir={blockDir} lang={blockLang} className="px-4 py-3 text-sm text-gray-700 align-top bidi-text">
    ...
    if (trimmedBlock.startsWith('#### ')) {
      return <h5 key={index} dir={blockDir} lang={blockLang} className="text-base font-semibold my-2 text-gray-700 bidi-text">{renderInlineFormatting(trimmedBlock.substring(5))}</h5>;
    }
    if (trimmedBlock.startsWith('### ')) {
      return <h4 key={index} dir={blockDir} lang={blockLang} className="text-lg font-semibold my-3 text-gray-800 bidi-text">{renderInlineFormatting(trimmedBlock.substring(4))}</h4>;
    }
    if (trimmedBlock.startsWith('## ')) {
      return <h3 key={index} dir={blockDir} lang={blockLang} className="text-xl font-semibold my-4 text-blue-900 bidi-text">{renderInlineFormatting(trimmedBlock.substring(3))}</h3>;
    }
    if (trimmedBlock.startsWith('# ')) {
      return <h2 key={index} dir={blockDir} lang={blockLang} className="text-2xl font-bold my-4 text-blue-900 bidi-text border-b border-gray-200 pb-2">{renderInlineFormatting(trimmedBlock.substring(2))}</h2>;
    }
    ...
    if (trimmedBlock.startsWith('* ') || trimmedBlock.startsWith('- ')) {
      ...
      return (
        <ul key={index} dir={blockDir} lang={blockLang} className="list-disc pl-6 my-2 space-y-1 text-gray-800">
          {listItems.map((li, liIndex) => <li key={liIndex} dir={blockDir} lang={blockLang} className="bidi-text">{renderInlineFormatting(li)}</li>)}
        </ul>
      );
    }
    ...
    if (/^\d+[\.\)]\s/.test(trimmedBlock)) {
      ...
      return (
        <ol key={index} dir={blockDir} lang={blockLang} className="list-decimal pl-6 my-2 space-y-1 text-gray-800">
          {listItems.map((li, liIndex) => <li key={liIndex} dir={blockDir} lang={blockLang} className="bidi-text">{renderInlineFormatting(li)}</li>)}
        </ol>
      );
    }
    ...
    // Handle Paragraphs
    if (trimmedBlock) {
        return <p key={index} dir={blockDir} lang={blockLang} className="my-2 text-gray-800 bidi-text">{renderInlineFormatting(trimmedBlock)}</p>;
    }
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  return (
    <div className={`flex my-4 ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        dir={isUser ? textDir : undefined}
        lang={isUser ? textLang : undefined}
