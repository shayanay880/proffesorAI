...
import { Message, Reference, Settings, Flashcard, GeneratedCase, AiModel } from '../types';
...
const PRIMARY_MODEL: AiModel = 'gemini-3-pro-preview'; 
const FALLBACK_MODEL: AiModel = 'gemini-3-pro-preview';

// Reduced context limits to prevent large payloads from causing network timeouts
const MAX_CONTEXT_MESSAGES = 12;
const MAX_CONTEXT_CHARS = 15000;

// Medication safety constants
const MEDICATION_KEYWORDS = [
    'acetaminophen', 'paracetamol', 'ibuprofen', 'naproxen', 'ketorolac',
    'amoxicillin', 'amox', 'azithromycin', 'ceftriaxone', 'vancomycin',
    'heparin', 'enoxaparin', 'warfarin', 'apixaban', 'rivaroxaban',
    'insulin', 'metformin', 'lisinopril', 'amlodipine', 'albuterol',
    'salbutamol', 'epinephrine', 'adrenaline', 'naloxone', 'ondansetron',
    'diphenhydramine', 'morphine', 'fentanyl', 'midazolam', 'propofol'
];
const DOSAGE_REGEX = /\b\d+(?:\.\d+)?\s?(mg|mcg|g|iu|units|ml|mL|microgram|milligram)\b/iu;
...
type MedicationSafetyGap = 'ageContext' | 'maxDose' | 'contraindications';

const detectMedicationMentions = (text: string) => {
    const lowered = text.toLowerCase();
    const matches = new Set<string>();

    MEDICATION_KEYWORDS.forEach(keyword => {
        if (lowered.includes(keyword)) matches.add(keyword);
    });

    const hasDoseSignal = DOSAGE_REGEX.test(text);
    const flagged = matches.size > 0 || hasDoseSignal;

    return { flagged, meds: Array.from(matches) };
};

const evaluateMedicationGaps = (text: string): MedicationSafetyGap[] => {
    const gaps: MedicationSafetyGap[] = [];
    const ageContextRegex = /(adult|pediatric|paediatric|child|children|peds|neonate|infant|adolescent|geriatr|mg\/kg|mcg\/kg|weight-?based)/i;
    const maxDoseRegex = /(max(?:imum)?\s+dose|do\s+not\s+exceed|ceiling\s+dose|upper\s+limit|daily\s+limit)/i;
    const contraindicationRegex = /(contraindicat|avoid\s+in|do\s+not\s+use|black\s+box|boxed\s+warning|pregnan|renal\s+failure|hepatic\s+failure|allergy|anaphylaxis)/i;

    if (!ageContextRegex.test(text)) gaps.push('ageContext');
    if (!maxDoseRegex.test(text)) gaps.push('maxDose');
    if (!contraindicationRegex.test(text)) gaps.push('contraindications');

    return gaps;
};

const buildSafetyNote = (gaps: MedicationSafetyGap[], meds: string[]) => {
    const missingPieces = gaps.map(gap => {
        if (gap === 'ageContext') return 'explicit adult vs pediatric dosing context';
        if (gap === 'maxDose') return 'maximum safe dose';
        return 'key contraindications';
    });

    const medLabel = meds.length ? ` for ${meds.slice(0, 4).join(', ')}` : '';
    return `⚠️ Safety check${medLabel}: medication guidance is missing ${missingPieces.join(', ')}. Verify age group, maximum recommended dose, and contraindications before applying.`;
};

const requestMedicationSafetyAddendum = async (
    originalText: string,
    meds: string[],
    gaps: MedicationSafetyGap[],
    modelName: string
): Promise<string> => {
    const missingPieces = gaps.map(gap => {
        if (gap === 'ageContext') return 'adult vs pediatric context';
        if (gap === 'maxDose') return 'maximum safe dose';
        return 'contraindications';
    }).join(', ');

    const focusMeds = meds.length ? meds.slice(0, 5).join(', ') : 'the medications mentioned above';
    const prompt = `You wrote the following clinical answer:\n${originalText}\n\nIt references ${focusMeds} but it lacks: ${missingPieces}.\nCreate a brief "Safety Addendum" (<=120 words) that ONLY supplies the missing pieces. Use crisp bullet points, preserve the original language, and do not restate the full answer.`;
    ...
    try {
        ...
        const addendum = result.text?.trim();
        if (addendum) {
            return `${originalText}\n\n**Safety Addendum:**\n${addendum}`;
        }
        return originalText;
    } catch (e) {
        console.warn('Safety addendum generation failed', e);
        return originalText;
    }
};

const applyMedicationSafetyGuardrails = async (
    text: string,
    settings?: Settings,
    modelName?: string
): Promise<string> => {
    const detection = detectMedicationMentions(text);
    if (!detection.flagged) return text;

    const gaps = evaluateMedicationGaps(text);
    if (gaps.length === 0) return text;

    const modelToUse = modelName || settings?.aiModel || PRIMARY_MODEL;
    try {
        const augmented = await requestMedicationSafetyAddendum(text, detection.meds, gaps, modelToUse);
        const remainingGaps = evaluateMedicationGaps(augmented);
        if (remainingGaps.length === 0) {
            return augmented;
        }

        const safetyNote = buildSafetyNote(remainingGaps, detection.meds);
        return `${safetyNote}\n\n${augmented}`;
    } catch (error) {
        console.warn('Medication safety completion failed:', error);
        const safetyNote = buildSafetyNote(gaps, detection.meds);
        return `${safetyNote}\n\n${text}`;
    }
};
...
export const generateResponse = async (
    ...
    const selectedModel: AiModel = settings?.aiModel ?? PRIMARY_MODEL;
    const secondaryFallback: AiModel = selectedModel === 'gemini-3-pro-preview' ? 'gemini-3-flash-preview' : 'gemini-3-pro-preview';
    ...
    try {
        ...
        const performGeneration = async (modelName: string, useGrounding: boolean) => {
            ...
            const normalizedObjectives = learningObjectives.map(obj => obj.trim()).filter(Boolean);
            const finalInstruction = `${SYSTEM_INSTRUCTION}\n\n${getSystemInstruction(currentSettings, normalizedObjectives)}`;
        ...
        try {
            result = await performGeneration(selectedModel, true);
        } catch (e1: any) {
            console.warn(`Attempt 1 (${selectedModel} + Grounding) failed:`, e1);
            
            if (isRetryableError(e1)) {
                try {
                    console.log(`Attempt 2: Retrying ${selectedModel} without grounding...`);
                    result = await performGeneration(selectedModel, false);
                } catch (e2: any) {
                    console.warn(`Attempt 2 (${selectedModel}) failed:`, e2);
                    
                    if (isRetryableError(e2)) {
                         console.log(`Attempt 3: Falling back to ${secondaryFallback}...`);
                         result = await performGeneration(secondaryFallback, false);
                    } else {
        ...
        const initialText = result.text || "No response generated.";
        const safeText = await applyMedicationSafetyGuardrails(initialText, settings, selectedModel);
        ...
        } else if (errorString.includes('missing gemini api key')) {
...
export const generateCase = async (params: {
    ...
    const modelToUse: AiModel = params.aiModel ?? 'gemini-3-pro-preview';
    const backupModel: AiModel = modelToUse === 'gemini-3-pro-preview' ? 'gemini-3-flash-preview' : 'gemini-3-pro-preview';

    try {
        const runOnce = async (overridePrompt?: string, attempt = 1, modelName: AiModel = modelToUse): Promise<any> => {
            try {
                const result = await ai.models.generateContent({
                    model: modelName,
                    contents: {
                ...
                if (attempt <= 3 && isRetryableError(error)) {
                    console.warn(`generateCase attempt ${attempt} (${modelName}) failed, retrying in ${attempt}s...`, error);
                    await new Promise(resolve => setTimeout(resolve, attempt * 1000));
                    return runOnce(overridePrompt, attempt + 1, modelName);
                }
                if (modelName !== backupModel) {
                    console.warn(`generateCase attempts on ${modelName} failed, switching to backup model ${backupModel}...`);
                    return runOnce(overridePrompt, attempt + 1, backupModel);
                }
                throw error;
            }
        };
