...
export const generateResponse = async (messages: Message[], settings?: Settings): Promise<GeminiResponse> => {
    ...
    try {
        ...
        // 2. Primary Model (No Grounding) - if 1 fails with retryable error
        // 3. Fallback Model (No Grounding) - if 2 fails with retryable error
        ...
        } else if (errorString.includes('rpc failed') || errorString.includes('xhr error')) {
