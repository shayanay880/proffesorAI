  ...
  ({ onSendMessage, isLoading, value, onChange }, ref) => {
    ...
    return (
      <div className="relative group p-1 bg-white rounded-3xl shadow-2xl shadow-slate-200 border border-slate-200 ring-4 ring-slate-50 transition-all focus-within:ring-cyan-100 focus-within:border-cyan-300">
        <textarea
          ref={ref}
          className="w-full bg-transparent px-5 py-4 pr-16 resize-none focus:outline-none bidi-text text-slate-700 font-medium placeholder:text-slate-400"
          dir={textDir}
          rows={1}
        ...
        <button
          ...
          className="absolute right-2 bottom-2 bg-cyan-600 text-white rounded-2xl p-3 shadow-lg shadow-cyan-200 hover:bg-cyan-700 hover:scale-105 active:scale-95 disabled:bg-slate-200 disabled:shadow-none disabled:scale-100 transition-all duration-300"
        >
          {isLoading ? (
