...
const App: React.FC = () => {
  ...
  const [settings, setSettings] = useState<Settings>({ mode: 'Study', studyLoad: 'standard', strictMode: false });
  ...
  const handleSendMessage = async (input: string) => {
    ...
    if (!convo) {
      convo = { id: generateId(), title: input.slice(0, 30), messages: [], createdAt: Date.now(), updatedAt: Date.now() };
      setConversations(prev => [convo!, ...prev]);
      setCurrentConversationId(convo.id);
    }

    const userMessage: Message = { id: generateId(), role: 'user', text: input };
    const updatedConvo = { ...convo, messages: [...convo.messages, userMessage], updatedAt: Date.now() };
    ...
    try {
      const result = await generateResponse(updatedConvo.messages, settings);
      const modelMessage: Message = { id: generateId(), role: 'assistant', text: result.responseText, references: result.references };
      setConversations(prev => prev.map(c => c.id === updatedConvo.id ? { ...updatedConvo, messages: [...updatedConvo.messages, modelMessage] } : c));
    } catch (err) {
      console.error(err);
    } finally {
      setIsLoading(false);
...
// Standard export added to fix the "no default export" error in index.tsx
export default App;
