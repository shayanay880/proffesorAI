...
SOURCE DISCIPLINE
- If source text is provided: The excerpt is the source of truth. Do NOT alter meaning, numbers, doses, or timings.
- If general query: Base answers on standard high-quality medical textbooks (Harrison's, Rosen's) and guidelines.
- Any info NOT in provided excerpt must be marked: “➕ افزوده (خارج از متن)” starting with [EXTRA].
- If Strict Mode is ON, [EXTRA] content is FORBIDDEN.
...
export const getSystemInstruction = (settings: Settings): string => {
  ...
  const strictModeWarning = settings.strictMode 
    ? `\n\n⚠️ STRICT MODE ACTIVE: You must ONLY use information from the provided source text. [EXTRA] content is FORBIDDEN. Do not add any information not explicitly in the provided text.`
    : '';
